...
subprocess.call('grep PASS ../result/info_fromSNPeff.target.pdb_and_residue.txt > Snp_PDB_info.txt', shell=True)
...
while line != '':
    ...
    ENST = info[9]
    AA_pos = info[11]
    AA_pos = re.findall('\d+',AA_pos)
    AA_pos = AA_pos[0]
    newline = chr + '\t' + g_pos + '\t' + AA_pos + '\t' + ENST + '\t' + PDBid + '\t' + ref + '\t' + chain + '\t' + pos + '\t' + alt + '\n'
