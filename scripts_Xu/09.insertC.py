...
while line != '':
    ...
    ENST = info[7]
    uniprot = info[8]
    sql = "INSERT INTO coordinate(Chr,g_pos,qual,DP,ref_base,alt_base,ENSG,ENST,Uniprot) VALUES ('%s','%d','%f','%s','%s','%s','%s','%s','%s')" % (Chr,pos,qual,DP,ref_b,alt_b,ENSG,ENST,uniprot)
    c.execute(sql)
    line = c_file.readline()'''
