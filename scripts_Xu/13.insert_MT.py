...
for line in lines:
    if line != '\t\n':
        ...
        if line[5] == 'NA':
            ...
            Sift2 = float(line[6])
        Sift_effect = line[7]
        sql = "INSERT INTO MT(ENST,Chr,g_pos,AA_pos,ref_AA,alt_AA,Sift_score,Sift_median,Sift_effect) VALUES ('%s','%s','%d','%d','%s','%s','%f','%f','%s')" % (ENST,Chr,pos,AA_pos,ref_AA,alt_AA,Sift1,Sift2,Sift_effect)
        c.execute(sql)
Sift_file.close()
...
while line != '':
    ...
    if n > 0:
        sql_update = """UPDATE MT set pph_prob = '%f',pph_FPR = '%f',pph_TPR = '%f',Polyphen_effect = '%s' where g_pos = '%d' """ % (P1,P2,P3,Pe,pos)
        c.execute(sql_update)
    else:
        sql_insert = """INSERT INTO mt(Chr,g_pos,pph_prob,pph_FPR,pph_TPR,Polyphen_effect) values ('%s','%d','%f','%f','%f','%s')""" % (Chr,pos,P1,P2,P3,Pe)
...
while line != '':
    ...
    n = c.execute(sql_select)
    if n > 0:
        sql_update = """UPDATE MT set PDBid = '%s',chain = '%s',PDB_pos = '%d',DDG ='%f' where ENST = '%s' and g_pos = '%d' """ % (PDBid,chain,PDB_pos,DDG,ENST,g_pos)
        c.execute(sql_update)
    else:
        sql_insert = """INSERT INTO MT(ENST,Chr,AA_pos,g_pos,ref_AA,alt_AA,PDBid,chain,PDB_pos,DDG) values ('%s','%s','%d','%d','%s','%s','%s','%s','%d','%f')""" % (ENST,Chr,AA_pos,g_pos,ref_AA,alt_AA,PDBid,chain,PDB_pos,DDG)
        c.execute(sql_insert)
